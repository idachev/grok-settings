#!/bin/bash
# Warn with the same YAD popup as ~/bin/claude-input-notify.sh when this
# session's context window is at or above CONTEXT_WARN_PERCENT.
...
#
# Wired as a Grok Stop hook (~/.grok/hooks/context-usage-warn.json).
...
NOTIFY_SCRIPT="${CONTEXT_WARN_NOTIFY:-$HOME/bin/claude-input-notify.sh}"
